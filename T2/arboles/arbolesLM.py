    ...
    if nuevo_nodo_Insertar != 0:     # si no entra un 0 que era terminar de insertar:
        if len(arbol_Insertar) == 0:   # si el arbol no tiene nada, está completamente vacío
            ...
            if nuevo_nodo_Insertar < arbol_Insertar[ubicacion_Insertar]['valor']:
                # es vacío el subarbol izquierdo->inserto datos
                if EsVacio(arbol_Insertar, arbol_Insertar[ubicacion_Insertar]['hijo_iz']) == True:
                    nuevo_nodo_Insertar = Constructor(nuevo_nodo_Insertar)
                    arbol_Insertar.append(nuevo_nodo_Insertar)
                    ...
                    for i in range(len(arbol_Insertar)):
                        if arbol_Insertar[i]['valor'] == arbol_Insertar[ubicacion_Insertar]['hijo_iz']:
                            pos_insertar = i
                    # inserto (recursivo) del subarbol izquierdo, indicado en la posición y con el valor a insertar
...
def Buscar(arbol_Buscar, valor_Buscar):
    ...
    if encontrado_buscar == True:
        print('Nodo encontrado: ' + '\033[32m' +
              str(encontrado_buscar) + '\033[0m')
        for i in range(len(arbol_Buscar)):
            # Buscamos el nodo y mostramos su padre e hijos
            if arbol_Buscar[i]['valor'] == valor_Buscar:
                print('Padre: \033[34m' + str(arbol_Buscar[i]['padre']) + '\n\033[0mHijo izquierdo: \033[34m' + str(
                    arbol_Buscar[i]['hijo_iz']) + '\n\033[0mHijo derecho: \033[34m' + str(arbol_Buscar[i]['hijo_dr']))
        for i in range(len(arbol_Buscar)):  # Buscamos el nodo otra vez
            if arbol_Buscar[i]['valor'] == valor_Buscar:
                # En caso de que no haya padre, no busca el hermano
                if arbol_Buscar[i]['padre'] != 0:
                    for j in range(len(arbol_Buscar)):  # Buscamos el padre del nodo
                        if arbol_Buscar[j]['valor'] == arbol_Buscar[i]['padre']:
                            # Si el nodo no coincide con su padre_hijo_izquierda, lo mostramos
                            if arbol_Buscar[j]['hijo_iz'] != encontrado_buscar:
                                print(
                                    '\033[0mHermano: \033[34m' + str(arbol_Buscar[j]['hijo_iz']) + '\033[0m')
                            else:
                                print(
                                    '\033[0mHermano: \033[34m' + str(arbol_Buscar[j]['hijo_dr']) + '\033[0m')

    else:
        # Nodo no encontrado
        print('Nodo encontrado: ' + '\033[31m' +
              str(encontrado_buscar) + '\033[0m')
...
def Visualizar(arbol_Visualizar):  # Función para ver el árbol
    ...
    for i in range(len(arbol_Visualizar)):
        print('\033[0mValor: \033[34m' + str(arbol_Visualizar[i]['valor']) + '\t\t' + '\033[0m        Predecesor: \033[34m' + str(arbol_Visualizar[i]['padre']) + '\t\t' +
              '\033[0m        Hijo Izquierda: \033[34m' + str(arbol_Visualizar[i]['hijo_iz']) + '\t\t' + '\033[0m        Hijo Derecha: \033[34m' + str(arbol_Visualizar[i]['hijo_dr']))
...
def NumeroHojas(arbol_NumeroHojas):  # Función que cuenta el número de hojas
    if len(arbol_NumeroHojas) > 0:  # Si el árbol está creado
        ...
        for i in range(len(arbol_NumeroHojas)):
            # Buscamos los valores que no tengan hijos
            if arbol_NumeroHojas[i]['hijo_iz'] == None and arbol_NumeroHojas[i]['hijo_dr'] == None:
                # Los guardamos en la lista de hojas
                lista_hojas.append(arbol_NumeroHojas[i]['valor'])
        lista_hojas.sort()  # Ordenamos la lista de menor a mayor
        # Convertimos los números enteros a strings (Para separarlos por comas sin que aparezca una al final)
        lista_hojas = [str(x) for x in lista_hojas]
        print('\nEl número de hojas es \033[34m' + str(
            len(lista_hojas)) + '\033[0m y son los nodos: \033[34m')
...
def NumeroNodos(arbol_NumeroNodos):  # Función que cuenta el número de nodos
    print('\nEl número de nodos que contiene el árbol es \033[34m' + str(
        len(arbol_NumeroNodos)) + '\033[0m')
...
while prog == True:
    ...
    while prog1 == True:
        ...
        while prog_seleccion < 1 or prog_seleccion > 7:
            ...
            prog_seleccion = int(
                input('\033[0;3;1m¿Qué quieres hacer?: \033[0m'))
        ...
        if prog_seleccion == 1:  # Generar un árbol o insertar más nodos
            ...
            while nodo != 0:  # con 0 se sale de insertar
                ...
                # inserto en un árbol, en una posición, el valor del nodo
                Insertar(arbol, 0, nodo)
                Visualizar(arbol)
                visualizacion = True
